...
import org.apache.flink.table.api.{EnvironmentSettings, FieldExpression, Table, Tumble, WithOperations}
...
object TestWindowByTableAPI {
  ...
  def main(args: Array[String]): Unit = {
    ...
    var table: Table = tableEnv.fromDataStream(stream)
    ...
    //这是过时写法：.select('sid, 'window.start, 'window.end, 'sid.count) //聚会计算
    //打印结果
